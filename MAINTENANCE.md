...
- **Chrome 不會渲染 JSON 網址，它會下載。** 8/13 台股 OpenAPI 三端點 `navigate` 後 `innerText` 全空、連續失敗逾五次。**JSON／CSV 端點一律用 `javascript_tool` 跑 `await fetch(url).then(r=>r.text())`**，不要 navigate 過去。
...
- **v17＋v18 欄位已於 8/11 首輪驗收通過**（thermo 帶判讀、snap 數值齊全、pulse 五行有理由、watchReview 敢寫落空並自承答錯、thread 三條）。**規格用語不需要調整**，後續只需抽查 thread 代號有沒有沿用。
- ~~Barron's 觀察中~~ **已結案（8/13）**：8/12 交回 2 則、8/13 交回 7 則，確認 8/11 的 0 則是工具失效而非來源問題。**MarketWatch 8/11–8/12 連兩天掛零、8/13 恢復 2 則，同樣結案。**
- ~~8/14 起要看分頁所有權的效果~~ **已驗收（8/14）**：呼叫數 432（8/13 為 1,785）、每則 7.8 萬（先前最佳 16.2 萬）、**被導走 0 次、重建 0 次**。分頁所有權是這半個月最有效的一條規則，**不要再動它**。
- **「卡片加長 +50% ≈ 全輪 +2%」這條預估至今未被驗證**：8/15、8/16 文字量確實成長約 50%（54,854→82,030 字），但**那兩天沒有附用量檔**。下一份用量檔要專門看這條——若實際超過 +5%，要回頭查是不是連帶推高了別的東西（深度卡一起變長、或採集端為了寫更長而多讀）。**同時盯完成時間**：加長後仍在 09:45 前完成，逼近 11:30 才需要把深度卡降到 6 張。
- **8/16 起 `check.py` 會印字數統計**（一般卡 650–800、深度卡 900–1,200，權威值在 `LEN_REG`／`LEN_DEEP`）。**只警告不擋是刻意的**——為了湊字數注水比超標更糟。看到「★偏短」時先問是不是素材真的稀薄，不要直接叫 writer 加長。
- **EIA 週報、EIA 天然氣、Baker Hughes 三個官方數據頁每天會觸發「疑似列表頁」警告**，人工確認放行即可。一天三筆、成本很低，暫不加白名單；若日後變多再考慮比照 `DEDUP_EXEMPT` 處理。
- **模型歸因從 8/14 起才有資料**：`run` 欄開始記錄執行模型。**在累積三到五天的乾淨基準線之前，不要動「採集子代理換便宜模型」這件事**——否則又會變成兩個變因同時動，8/11 那次就是這樣失去歸因能力。
- **採集子代理換模型是已知的下一個大槓桿但尚未執行**：8/13 子代理佔全場 77%，而採集是機械工作、寫稿才是判斷工作。風險是採集員身上有三件非機械的事（付費牆內文量判定、擴充套件注入偵測、工具失效 vs 被擋的區分），**退化的方式是安靜的**。要測就單獨測一天並比對這三項的回報品質。
...
- ~~華爾街見聞觀察中~~ **已結案（8/11）**：8/11 交回 12 則、為 8/09 以來最高，8/10 的全站 0 則確認為單日事件，不需處置。
