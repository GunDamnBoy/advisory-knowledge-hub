...
- **Barron's 觀察中（8/11 起）**：當天 0 則，但原因是 `javascript_tool` 整個 session 失效、輪詢跑不起來，**不是來源問題**——它 7/30 以來每天都有產出。連三天 0 則才處置，判斷依據是 `metrics.py --src`。
- **8/12 起要看分批派工的效果**：盯每則成本有沒有從 34.5 萬回到 25 萬以下、D／E／F 三組的回合數有沒有降下來、有沒有人回報分頁爭奪或熔斷。若分批後仍卡，下一步才考慮降到 5 組。
