...
- **其餘則**：一般 `.card` ＋ `.lead`（**450～600 字**中長摘要）＋ 2～3 條重點 bullet。

  **字數在 2026/08/14 由 250～350 提高到 450～600**，理由是實測出來的成本結構：那一輪**採集子代理佔 60.5%、真正讀網頁佔 12.3%，兩者都完全不受卡片長度影響**；撰寫輸出加寫入快取合計只佔 17.3%，其中還包含推理、總覽與心得。實測「卡片文字 +50% 約等於全輪 +2.0%」。**加長既有卡片只付「寫」的錢，多收一則卡片要付「採集＋寫」的錢，而採集是寫的三倍以上——想提高每輪的價值密度，加長比加量划算得多。**（8/14 未加長前的實測平均已是一般卡 454 字、深度卡 926 字。）
...
進度落後時**依序砍**：(1) 深度卡由 6～8 張降為 4 張，只保留當日最重要的四條主軸各一張；(2) 中長摘要由 450～600 字降為 250～350 字；(3) **次要組與衛星組**的閱讀篇數各減三分之一——**核心組（美股、AI、央行、台灣）不減**，那四組是全站骨幹。
