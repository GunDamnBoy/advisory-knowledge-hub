...
- **深度卡**：用 `.card.wide`（跨兩欄）＋ `.longread`（多段落、**900～1,300 字**的深度摘要）＋標 `.tag.deep`「深度」；深入說明事件來龍去脈與對市場的意義。
- **其餘則**：一般 `.card` ＋ `.lead`（**550～900 字**中長摘要）＋ 2～3 條重點 bullet。

  **本節只有這兩個區間，不要在別處找第二個數字（2026/08/17 收斂）。** 權威值在 `check.py` 的 `LEN_REG`／`LEN_DEEP`；歷次調整的沿革與依據全部在 `CHANGELOG.md`，本節不再重述。
