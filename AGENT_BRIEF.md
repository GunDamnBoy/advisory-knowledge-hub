...
`scripts/check.py` 會自動依 `window.from` 的星期切換並在輸出標明「模式：平日／週末」，**不需要人工判斷**。
...
進度落後時**依序砍**：(1) 深度卡由 6～8 張降為 4 張，只保留當日最重要的四條主軸各一張；(2) 中長摘要由 550～900 字降為 350～450 字；(3) **次要組與衛星組**的閱讀篇數各減三分之一——**核心組（美股、AI、央行、台灣）不減**，那四組是全站骨幹。
