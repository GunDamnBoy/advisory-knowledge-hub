...
- `about`：`timeliness`／`notes`（list）／`access`／`run`／`limits`
  - **`run` 欄第一句必須寫「本輪由 <模型名稱> 執行」**（2026/08/13 第 20 次修訂新增）。這套系統幾乎每天改規格，日後比較「哪一版比較好」時，**不知道中間換過模型就會把模型差異誤判成規格改壞了**——8/12 與 8/13 同一份規格、每則成本差 74%，現在已經無法回頭歸因。
    **取法：直接 `Grep` 已知的工作階段 `.jsonl` 抓 `"model"` 欄位，不要先 `Glob` 全域掃路徑**（2026/08/17 實測：Glob 掃回 100 條路徑、花掉 0.35M 有效 token，佔當輪主對話 8%——**那是純浪費，因為要找的欄位就在當前工作階段那一個檔裡**）。**模型名稱一律取自工作階段的 metadata（`.jsonl` 紀錄的模型欄位），不要憑對話中的自我描述填**——8/14 首日就寫錯：`run` 欄寫「Claude Opus 4.5（依工作階段標示）」，而工作階段紀錄實際是 `claude-opus-5`。**取不到 metadata 就寫「未確認」，不要猜也不要用近似的版本號**；寫「未確認」是可接受的，寫錯的版本號比空著更糟，因為它看起來像有效資料。
...
- **深度卡**：用 `.card.wide`（跨兩欄）＋ `.longread`（多段落深度摘要）＋標 `.tag.deep`「深度」；深入說明事件來龍去脈與對市場的意義。**全卡 900～1,300 字。**
- **其餘則**：一般 `.card` ＋ `.lead`（中長摘要）＋ 2～3 條重點 bullet。**全卡 550～900 字。**
- **降級時的一般卡**：**全卡 350～450 字**。只在第 5 節「進度落後時依序砍」那條觸發，正常情況不用。**降級那天 `check.py` 仍以 550～900 判定、照樣會報「★偏短」，那是預期的**——不要因此把它調回去。

  **「全卡」＝ `body` 加上全部 `bullets` 的字數總和**，與 `check.py` 的量法逐字相同（`len(body) + sum(len(b) for b in bullets)`）。**bullet 算在區間裡面，是全卡的一部分。** 2026/08/17 之前本節把數字掛在 `.lead` 上、bullet 另外列在後面，字面讀起來像是「lead 自己就要 550～900、bullet 再另計」——那比程式的量法多出 2～3 條 bullet 的量。**規格的數字要跟量它的那把尺同一把。**

  **本節是全檔唯一規定「一張卡該多長」的地方，數字與形容詞都是（2026/08/17 收斂）。** 權威值在 `check.py` 的 `LEN_REG`／`LEN_DEEP`；歷次調整的沿革與依據全部在 `CHANGELOG.md`，本節不再重述。**第 7 節只管語氣，不管長度。**
  **為什麼收斂**：8/17 之前本節同時列著四個歷代區間，只有一個還有效，而過期的那個還被加粗、排在有效值之後。**同一天 v23 prompt 把字數改成「見 brief 第 4 節」——等於把唯一來源指向一個有四個競爭數字的段落。** 當天實際跑出中位 504、59／82 低於下限，分布在下限附近出現尖峰（素材不足會產生左長尾，不會產生尖峰）。**一個規格有幾個數字，執行端就有機會挑錯一個**——所以過期值連「為了說明它過期」都不該再寫進本檔，要查沿革去 `CHANGELOG.md`。
  **形容詞跟數字一樣會被當成錨。** 8/17 只收斂了數值，性質形容詞還散在第 4 節末與第 7 節，其中一個站在下限的對面、而且位在全檔最後一行（近因位置最強）。**8/18 一併收斂**：長度的話語權整份收在本節。

  **`check.py` 每輪印出兩組字數統計**（張數、平均、逾越區間張數），**但只警告、不計入出口碼**：字數是品質指標不是正確性指標，**為了湊字數注水比超標更糟**。量測的目的是讓漂移被看見，不是拿數字逼稿——**看到「★偏短」時先問素材是不是真的薄，再看分布形狀**：左長尾多半是素材，尖峰多半是錨錯了。
...
**每則卡片**：來源徽章＋主題標籤（`.tag`，可 hot/warn/pos）＋日期＋標題（`h3`）＋一段詳細闡述（`.lead`）＋2～3 條重點 bullet（`.card ul`）＋原文連結（真實可點）＋溫度色條（`t-green`/`t-yellow`/`t-orange`/`t-red`）。**長度見本節開頭的兩個區間。**
...
3. **每日必抓的官方數據**（F 組與 C 組分工）：
   ...
   - **黃金（衛星組保底卡，2026/08/14 再次換源）**：金價（現貨與 COMEX 近月**要分開標**）＋下列**免登入**數據源：
     - **SPDR Gold Shares 每日持倉**——全球最大黃金 ETF，持倉噸數的日變化是**資金流向**的標準代理指標。**官網 `spdrgoldshares.com/usa/` 已改版為 SPA、`innerText` 取不到數字（2026/08/17 實測），改走 `api.spdrgoldshares.com` 端點**（同網域 fetch 規則同台股端點：先 navigate 到同網域頁面再 `javascript_tool` fetch）。**GLD 與 GLDM 兩檔都取**（8/14 實測兩檔皆可得）。
...
進度落後時**依序砍**：(1) 深度卡減為 4 張，只保留當日最重要的四條主軸各一張（正常張數見第 4 節）；(2) 中長摘要改用第 4 節的**降級區間**；(3) **次要組與衛星組**的閱讀篇數各減三分之一——**核心組（美股、AI、央行、台灣）維持原量**，那四組是全站骨幹。
...
心得有觀點但保持中立，是分析不是投資建議；版面可快速掃讀。（語言與讀者設定見本文件開頭。）

**本節不規定長度。** 卡片與心得的字數一律以第 4 節為準——**性質形容詞會被當成字數的錨用**，所以長度只在一個地方講。
