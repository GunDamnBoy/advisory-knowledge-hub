...
- **深度卡**：用 `.card.wide`（跨兩欄）＋ `.longread`（多段落、**約 900～1,200 字**的深度摘要，2026/08/16 由 800～1,000 追認）＋標 `.tag.deep`「深度」；深入說明事件來龍去脈與對市場的意義。
- **其餘則**：一般 `.card` ＋ `.lead`（**650～800 字**中長摘要）＋ 2～3 條重點 bullet。
...
進度落後時**依序砍**：(1) 深度卡由 6～8 張降為 4 張，只保留當日最重要的四條主軸各一張；(2) 中長摘要由 650～800 字降為 350～450 字；(3) **次要組與衛星組**的閱讀篇數各減三分之一——**核心組（美股、AI、央行、台灣）不減**，那四組是全站骨幹。
