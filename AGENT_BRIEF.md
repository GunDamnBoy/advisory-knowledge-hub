...
2. 用 Claude in Chrome 逐一讀 25 家當日重點（付費為主）。開 **7 個 subagent 平行分組**，每個先自己開新分頁再作業。

   **每份派工提示 ＝ `scripts/subagent_preamble.md` 全文（共用前言：任務邊界、預篩規則、讀法、合規紅線、回報格式）＋ `scripts/list_timestamps.js` 全文 ＋ `scripts/read_article.js` 全文 ＋ `window.from` 的值 ＋ 該組的來源與配額。** 共用段落已外部化（2026/08/09 第 15 次修訂），**不要在每份提示裡重寫一遍**——9 份提示中那三段原本幾乎逐字重複，佔全輪成本約 5.6%，且會連帶推高每個子代理每回合的快取讀取量。
