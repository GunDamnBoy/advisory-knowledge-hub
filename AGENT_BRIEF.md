...
- `about`：`timeliness`／`notes`（list）／`access`／`run`／`limits`
  - **`run` 欄第一句必須寫「本輪由 <模型名稱> 執行」**（2026/08/13 第 20 次修訂新增）。這套系統幾乎每天改規格，日後比較「哪一版比較好」時，**不知道中間換過模型就會把模型差異誤判成規格改壞了**——8/12 與 8/13 同一份規格、每則成本差 74%，現在已經無法回頭歸因。照工作階段當下實際使用的模型填，**不確定就寫「未確認」；寫「未確認」可以接受，猜一個填進去不行**。
...
3. **每日必抓的官方數據**（F 組與 C 組分工）：
   - **台股官方數據一律改走 JSON／CSV 端點，不要再開網頁版表單頁**（2026/08/06 第 11 次修訂，全部經實測驗證）。舊的 `mops.twse.com.tw/mops/web/*` 與 TPEx 網頁版**連續三輪被重導至新版 SPA 而失敗**，已確認是結構性問題，不要再試。
     ...
     **正確做法：停在任何一個已開啟的頁面，用 `javascript_tool` 直接 fetch**——
     ```
     await fetch('<端點網址>').then(r => r.text())
     ```
     拿到字串後自己 `JSON.parse`（CSV 端點直接切行）。這仍然是 Chrome 工具、走使用者的瀏覽器 session，**不違反「不要用 WebFetch／curl／Python」那條規則**（線上驗證步驟本來就是這樣做的）。`rwd` 那三個端點目前 `navigate` 仍可用，但**一律改走 fetch 比較安全，行為一致**。
