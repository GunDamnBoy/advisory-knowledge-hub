...
try:
    from check import QUOTA          # 下限表以 check.py 為唯一權威，不要在這裡再抄一份
except Exception:
    QUOTA={}
...
def main():
    ...
    if '--csv' in args:
        print('date,cards,groups,deep,sources,ts_cov,window_from,window_to')
        for d,cards in rows:
            w=d.get('window') or {}
            print('%s,%d,%d,%d,%d,%d,%s,%s'%(d['date'],len(cards),len({l for l,_ in cards}),
                sum(1 for _,c in cards if c.get('deep')),len({c['src'] for _,c in cards}),
                sum(1 for _,c in cards if c.get('ts')),w.get('from',''),w.get('to','')))
        return
    print('=== 每版總表 ===')
    print(f"{'日期':<11}{'則數':>5}{'組數':>5}{'深度':>5}{'來源':>5}{'ts%':>6}  {'窗口':<24}里程碑")
    for d,cards in rows:
        w=d.get('window') or {}
        ...
        print(f"{d['date']:<11}{n:>5}{len({l for l,_ in cards}):>5}"
              ...
              f"{(ts*100//n if n else 0):>5}%  {win:<24}{MILESTONES.get(d['date'],'')}")
    if '--groups' in args and QUOTA:
        ...
        for d,cards in rows:
            ...
            cells=''.join(f'{(str(cnt[l]-QUOTA[l]) if l in cnt else "—"):>7}' for l in labs)
            print(f"{d['date']:<11}{cells}")
        print('（"—"＝該版沒有這一組；數字＝實際則數減下限的緩衝）')
    if '--src' in args:
        print('\n=== 來源貢獻矩陣 ===')
